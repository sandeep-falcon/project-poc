...
import { debounceTime, delay, from, Observable, of, Subscription, switchMap } from 'rxjs';
import { MaterialModule } from '../../../material/material.module';

@Component({
  selector: 'app-switchmap',
  standalone: true,
  imports: [ReactiveFormsModule, AsyncPipe, MaterialModule],
...
export class SwitchmapComponent {
  ...
  $switchMap = from([1, 2, 3, 4]).pipe(switchMap(data => {
  ...
  // searchItem(query: number): Observable<any> {
  //   console.log('query', query);
  //   if (!query) {
  //     return of([]);
  //   }
  //   // return this.http.get<any>(`https://jsonplaceholder.typicode.com/todos/${query}`);
  //   const index = this.data.findIndex(item => item.id === query)
